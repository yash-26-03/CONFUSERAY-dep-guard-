"""Unit tests for the file parsers. Run with:  python -m pytest tests/"""
import os
import pytest
...
def test_requirements_basic_and_unpinned():
    ...
    # comments and option lines must not become fake packages
...
def test_version_is_higher():
    assert version_is_higher("999.0.0", "^2.0.0") is True
    assert version_is_higher("1.0.0", "==2.0.0") is False
    assert version_is_higher("8.0.0", "any") is True
    assert version_is_higher(None, "1.0.0") is False


def test_should_fail_thresholds():
    crit = Finding("x", "npm", "critical", "t", "d", "r")
    med = Finding("y", "npm", "medium", "t", "d", "r")
    assert should_fail([crit], "high") is True
    assert should_fail([med], "high") is False
    assert should_fail([med], "medium") is True
